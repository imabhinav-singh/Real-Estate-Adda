...
public class SalesReport {
	...
	public SalesReport() {
		...
		private void initialize(){
			...
			frame.setTitle("Agent's Sales Enquiry");
					...
					if(chckbxNewCheckBox.isSelected() || chckbxFilterByYear.isSelected()) {
						if((chckbxNewCheckBox.isSelected() && textField1.getText().equals("")) || (chckbxFilterByYear.isSelected() && textField.getText().equals(""))) {
							...
							if(chckbxFilterByYear.isSelected() && chckbxNewCheckBox.isSelected()) {
								...
								else sqlconnect.salesResultAgentYear(agentID, year, chckbxNewCheckBox_1.isSelected(), chckbxSortByDate.isSelected());
							}
							else if(chckbxNewCheckBox.isSelected()) {
								SQLConnect sqlconnect = new SQLConnect();
								String agentID = textField1.getText();
								if(!sqlconnect.verifyAgent(agentID)) JOptionPane.showMessageDialog(frame, "Invalid ID");
								else sqlconnect.salesResult(agentID, chckbxNewCheckBox_1.isSelected(), chckbxSortByDate.isSelected());								
							}
							else {
								SQLConnect sqlconnect = new SQLConnect();
								String year = textField.getText();
								sqlconnect.salesResultYear(year, chckbxNewCheckBox_1.isSelected(), chckbxSortByDate.isSelected());
							}
						}
					}
					else {
			...
			chckbxFilterByYear.setBackground(new Color(0, 102, 102));
			...
			chckbxSortByDate.setBackground(new Color(0, 102, 102));
